...
void senderLoRA(){
	...
	for (int i = 0; i < LoRAtoSend.header->nbOf_packet; i++){
		...
		memcpy(buffer + sizeof(LORA_HeaderforSending),LoRAtoSend.payload->buffer + i * RFM9x_FIFO_SIZE-sizeof(LORA_HeaderforSending), len_payload ); //On oublie surement encore le header
	...
	vPortFree(LoRAtoSend.itemFromUBXtoFree);
	vPortFree(LoRAtoSend.gnssReturntoFree);
	vPortFree(LoRAtoSend.header);
	vPortFree(LoRAtoSend.LoRAreceivetoFree);
