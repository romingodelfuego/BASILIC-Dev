...
void MX_FREERTOS_Init(void) {
  ...
  osMessageQDef(UARTdebug, 4, UARTdebugQ_t);
  ...
  osThreadDef(StartInit, StartInit_TASK, osPriorityRealtime, 0, 128);
  StartInitHandle = osThreadCreate(osThread(StartInit), NULL);

  /* definition and creation of SenderLoRA */
  osThreadDef(SenderLoRA, SenderLoRA_TASK, osPriorityNormal, 0, 128);
  SenderLoRAHandle = osThreadCreate(osThread(SenderLoRA), NULL);

  /* definition and creation of ReceiverLoRA */
  osThreadDef(ReceiverLoRA, ReceiverLoRA_TASK, osPriorityNormal, 0, 128);
...
void StartInit_TASK(void const * argument)
{
  /* USER CODE BEGIN StartInit_TASK */
	...
	for(;;)
	{
		...
		osThreadTerminate(StartInitHandle);
	}
